...
```mermaid
flowchart LR
  A["Document<br/>(PDF/DOCX/PPT)"] --> B["Parse / Extract<br/>(text, images, tables)"]
  B --> C["Chunk / Normalize<br/>(text chunks + asset records)"]
  C --> D["Summarize assets (optional)<br/>(images/tables)"]
  C --> E["Embed text chunks"]
  D --> F["Embed summaries"]
  E --> G["Vector Index<br/>(FAISS)"]
  F --> G
  G --> H["Retrieve Top-K"]
  H --> I["LLM Answer + citations"]
  I --> J{"Post-filter assets?"}
  J -->|yes| K["Filter by cited pages/window"]
  K --> L["Final response<br/>(text+tables+images+cites)"]
  J -->|no| L
